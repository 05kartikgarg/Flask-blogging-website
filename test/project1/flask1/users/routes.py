...
def register():
	...
	if form.validate_on_submit():
		...
		db.session.commit()
		flash(f'Account has been created succesfully !','success')
...
def login():
	...
	if form.validate_on_submit():
		...
		if user1 and bcrypt.check_password_hash(user1.password,form.password.data):
			...
			flash('Login Unsuccessfull. Please check the email and password!')
...
def user_posts(username):
    ...
    posts = Post.query.filter_by(author=user1)\
        .order_by(Post.date_posted.desc())\
        .paginate(page=page, per_page=5)
    return render_template('user_posts.html', posts=posts, user=user1)
